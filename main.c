...
mid++;
int h = 1;
struct Airplane*c = (struct Airplane*)malloc(size * sizeof(struct Airplane));
int step;
while (h < size) {
step = h;
int i = 0;
...
int main() {
    ...
    struct Airplane* ar = (struct Airplane*) malloc(size * sizeof(struct Airplane));

    for (size_t i = 0; i < size; i++) {
        scanf("%20s %20s %20s %d %d", ar[i].brand, &ar[i].model,
              &ar[i].appointment, &ar[i].crew_number, &ar[i].range);
    }

    merge_sort(ar, size, is_less);

    for (size_t i = 0; i < size; i++) {
        printf("%s %s %s %d %d\n", ar[i].brand, ar[i].model,
               ar[i].appointment, ar[i].crew_number, ar[i].range);
    }
    return 0;
}